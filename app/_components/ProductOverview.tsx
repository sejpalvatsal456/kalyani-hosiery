...
export default function ProductOverview({
  ...
  const [isOutOfStock, setIsOutOfStock] = useState<boolean>(productData.variety[0].sizes[0].stock === 0);
  ...
  const handleClick = () => {
    ...
    alert(`Color: ${selection.color} | Size: ${selection.size} | Out of Stock: ${isOutOfStock}`);
    setIsLoading(false);
  };

  const sizeListEl = productData.variety[selectedColor].sizes.map(({ size, stock }, key) => {
    return (
      <button
        key={key}
        className={
          ...
          (selectedSize == key
            ? `border-none bg-[#fc2167] text-white`
            : "")
        }
        onClick={() => {
          setSelectedSize(key)
          setIsOutOfStock(productData.variety[selectedColor].sizes[key].stock === 0)
        }}
      >
        {size}
      </button>
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      ...
                      setSelectedSize(0);
                      setIsOutOfStock(productData.variety[selectedColor].sizes[0].stock === 0)
                    }}
                    style={{
                      boxShadow:
