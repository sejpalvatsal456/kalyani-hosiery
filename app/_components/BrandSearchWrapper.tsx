...
import { formatLabel } from "./SubcategoryButton";
...
const getLowestPrice = (product: DisplayProductType) => {
  return Math.min(
    ...product.variety.flatMap((v) => v.sizes.map((s) => s.sellingPrice)),
  );
};

export default function BrandSearchWrapper({
  brandName,
}: {
  brandName: string;
}) {
